...
class MemoryManager:
    ...
    async def read_memory(self, identifier: str) -> Optional[Dict[str, Any]]:
        ...
        if os.path.exists(identifier) and identifier.endswith('.md'):
            ...
            title = self._extract_title_from_content(content) or os.path.splitext(os.path.basename(filepath))[0]
        ...
        title_match = identifier
        if title_match.lower() in self._memory_index:
            normalized_title = self._memory_index[title_match.lower()]
            
            # Find file by normalized title
            for folder in os.listdir(self.memory_dir):
                ...
                for filename in os.listdir(folder_path):
                    ...
                    extracted_title = self._extract_title_from_content(content) or os.path.splitext(filename)[0]
                    if extracted_title == normalized_title:
                        return {
        ...
        # Try fuzzy matching by sanitized filename
        ...
        for folder in os.listdir(self.memory_dir):
            ...
            for filename in os.listdir(folder_path):
                ...
                if name_without_ext == sanitized_identifier:
                    filepath = os.path.join(folder_path, filename)
                    with open(filepath, 'r', encoding='utf-8') as f:
                        content = f.read()
    ...
    async def search_memories(self, 
        ...
        # Prepare query variations to increase match chances
        ...
        if words:
            ...
            query_variations.append(capitalized)
            
            # For short queries (likely entity names), add just the main word capitalized
            if len(words) <= 2:
                for word in words:
                    if len(word) > 3:  # Only consider substantial words
                        query_variations.append(word.capitalize())
        ...
        for folder in os.listdir(self.memory_dir):
            ...
            for filename in os.listdir(folder_path):
                ...
                # Initialize score for this memory
                score = 0
                matched = False
                
                # Check direct query matches
                for variation in query_variations:
                    if variation in title_lower:
                        score += 50  # High score for title match
                        matched = True
                    if variation in content_lower:
                        score += 20  # Medium score for content match
                        matched = True
                ...
                for keyword in keywords:
                    if keyword in title_lower:
                        score += 30  # Good score for title keyword match
                        keyword_matches += 1
                        matched = True
                    elif keyword in content_lower:
                        score += 10  # Lower score for content keyword match
                        keyword_matches += 1
                        matched = True
                ...
                # Check for potential topic matches
                for topic in self._extract_topics_from_content(content):
                    if any(variation in topic.lower() for variation in query_variations):
                        score += 40  # High score for topic match
                        matched = True
                
                # Check for semantic matches in observations sections
                observations = self._extract_observations_from_content(content)
                for obs in observations:
                    # Observation semantic match (e.g. "created by" matches "who created")
                    if ("create" in query_lower and "create" in obs.lower()) or \
                       ("who" in query_lower and ("by" in obs.lower() or "author" in obs.lower())) or \
                       ("when" in query_lower and any(date_word in obs.lower() for date_word in ["date", "year", "time", "when"])):
                        score += 25  # Good score for semantic match
                        matched = True
    ...
    def _extract_keywords_from_query(self, query: str) -> List[str]:
        ...
        stop_words = {
            ...
            "have", "has", "had", "do", "does", "did", "about", "for", "in", "to", 
            "from", "by", "with", "about", "against", "between", "into", "through", 
            "during", "before", "after", "above", "below", "up", "down", "in", "out",
            "on", "off", "over", "under", "again", "further", "then", "once", "here", 
            "there", "when", "where", "why", "how", "all", "any", "both", "each", 
            "some", "more", "most", "other", "such", "no", "nor", "not", "only", 
            "same", "so", "than", "too", "very", "one", "every",
            "could", "should", "would", "can", "what", "which"
        }
    ...
    def _extract_topics_from_content(self, content: str) -> List[str]:
        """Extract topic titles from memory content."""
        topics = []
        
        # Try to find topic links [[Topic]] or direct 'about' relations
        pattern = r'\[\[(.*?)\]\]'
        topics.extend(re.findall(pattern, content))
        
        # Add topic from about relations
        about_pattern = r'- about \[\[(.*?)\]\]'
        topics.extend(re.findall(about_pattern, content))
        
        return topics
        
    def _extract_observations_from_content(self, content: str) -> List[str]:
        """Extract observation text from content."""
        observations = []
        observation_pattern = r'- \[(.*?)\](.*?)(?=\n|$)'
        
        for match in re.finditer(observation_pattern, content):
            observations.append(match.group(2).strip())
    ...
    async def update_memory(self, 
        ...
        if new_title and new_title != original_title:
            ...
            if original_title.lower() in self._memory_index:
                del self._memory_index[original_title.lower()]
