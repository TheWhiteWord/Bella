...
async def detect_and_handle_memory_command(
    ...
    # Check for @agent Continue pattern
    continue_pattern = r"@agent\s+continue"
    if re.search(continue_pattern, text, re.IGNORECASE):
        response = await handle_continue_command(conversation_history)
